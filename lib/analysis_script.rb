...
hashed_lines.keys.each do |endpoint|
  ...
  puts "mode response time: #{mean_mode_median_hash["mode"][0].length > 0 ? mean_mode_median_hash["mode"][0].join(",") : 0} ms with frequency #{mean_mode_median_hash["mode"][1] != nil ? mean_mode_median_hash["mode"][1] : 0}"
  ...
  if camera_service_time_hash.keys.length > 0
